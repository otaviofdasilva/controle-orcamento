...
function token(email) {
    return jwt.sign({ id:`${email}::${new Date().toISOString()}` }, process.env.ASSINATURA, { expiresIn: "15m" });
}
...
export default async function acesso(app, m) {
    ...
    app.route("/api/usuarios/logout")
       .post(passport.authenticate("bearer", { session: false })
            , async function (request, response) {
                  console.log(new Date().toISOString(), request.url, request.headers);
