...
async function altera({ email, password }) {
  const r = await q(
    `UPDATE usuarios SET hash  = $2 WHERE  email = $1`,
    email,
    password
  );
...
async function verificaEmail(email) {
  const { rowCount, rows } = await q(
    `SELECT email, hash FROM usuarios WHERE email = $1 LIMIT  1`,
    email
  );
...
async function seleciona({ email, password }) {
  ...
  if (!usuario) return Promise.resolve(null);
  if (await bcrypt.compare(password, usuario.hash))
    return Promise.resolve(email);
